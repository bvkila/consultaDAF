...
################################################################################
    ...
    NOME_TABELA_DAF = 'base_dados' #mudar isso aqui futuramente
    NOME_TABELA_CONTAB = 'teste_DAF_observacao'
...
################################################################################
    ...
    if not df_DAF.empty:
        df_DAF.to_sql('base_dados', con, if_exists='append', index=False)
    ...
    if not df_contab.empty:
        df_contab.to_sql('teste_DAF_observacao', con, if_exists='append', index=False)
