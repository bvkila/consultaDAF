...
class Database:
    ...
    def inserir_operacao(self, subcategoria, valor, data):
        try:
            cursor = self.conn.cursor()
            cursor.execute("INSERT INTO externos (subcategoria, valor, data) VALUES (?, ?, ?)",
    ...
    def atualizar_operacao(self, id_op, novo_valor, nova_data):
        try:
            cursor = self.conn.cursor()
            cursor.execute("UPDATE externos SET valor = ?, data = ? WHERE id = ?", 
    ...
    def excluir_operacao(self, id_op):
        try:
            cursor = self.conn.cursor()
            cursor.execute("DELETE FROM externos WHERE id = ?", (id_op,))
            self.conn.commit()
            return True
        except Exception as e:
